...
<section id="choixclasse">	<span id="pwd"><input name="pwd" type="password">	<span id="erreur"></span><h4 id="valid">Valider</h4></span><span id="alphabetix">Alphabétique</span></section>
<section id="titres">
...
$('#resultats').on('click','button', function(){
	$points=$(this).siblings('.pts');
	nbpoints=parseInt($points.html());
...
$('#resultats').on('click','.nom', function(){
	...
	level=parseInt($level.html())+1;
	level=(level>5)? 0:level;
	$level.attr('data-level','actif').html(level);
});
...
$('#choixclasse').on('click','#save', function(){
	$('[data-modif]').each(function(index){
		...
		val=$(this).html();
		datas={'ID':$(this).parents('section').attr('id'),'valeur':$(this).html()};//console.log(datas);
...
function affichedonnees(datas){
	...
	//groupe=0;
			...
			html+='<section id="'+infosgpe[groupe]['IDgpe']+'"><div class="groupe"><span class="pts">'+infosgpe[groupe]['points']+'</span><h4>'+groupe+'</h4>';
	...
	balises='<article class="moygpe">'; 
				...
				for(var i in IDtodisplay){
					...
					balises+='<div class="r'+ID+'">';
						...
						balises+='<span style="background:'+couleur(Math.round(it*100))+';">'+k+'</span>';		
						}
					balises+='</div>';
				}		
...
htm='';
		if(alphatix){htm+='<section><div class="groupe">Groupe'+infos['groupe']+'('+infosgpe[infos['groupe']]['points']+'pts)';}
				...
				htm+='<div class="nom" ><span class="level">0</span>'+infos['Nom']+'<span class="rsltinst"></span></div>';
...
console.log(infos);
	...
	for(var i in IDtodisplay){
		...
		for(var k=0;k<10;k++){j=k+1;rslt=parseInt(its[k]);
			...
			else if(its[k]=='6'){txt+=' class="validtr"';
			}	
			moy[1]++;
			txt+='>'+j+'</span>';
		}
